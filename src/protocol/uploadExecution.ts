...
interface UploadRequest {
  method: string
  url: string
  part: { name: string; fileName: string; contentId: string }
}

type UploadExecutionResult =
  | { ok: true; requests: UploadRequest[]; fieldValue: string | string[] | undefined }
  | { ok: false; code: string; fileIndex: number; requests: UploadRequest[]; fieldValue: null }

function failure(code: string, fileIndex: number, requests: UploadRequest[] = []): UploadExecutionResult {
  return { ok: false, code, fileIndex, requests, fieldValue: null }
}

function matchesAccept(file: UploadFile, accept: string | undefined): boolean {
...
function requestFor(action: UploadAction, file: UploadFile): UploadRequest {
  return {
...
function responseValue(response: UploadResult['response']): string | null {
...
export function executeUpload(input: UploadInput): UploadExecutionResult {
  ...
  return { ok: true, requests, fieldValue: action.multiple ? values : values[0] }
}